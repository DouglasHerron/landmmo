/**
 * DORG_Main.js
 * Warrior entrypoint — Run THIS slot on Dorg (Stop first).
 */
(function () {
    "use strict";
    ...
    root.BOT_STATUS = { main: "DORG_Main", phase: "starting" };
    log("DORG_Main starting...");
    try {
        if (typeof set_message === "function") set_message("DORG boot");
    } catch (e) { /* ignore */ }
    ...
    try {
        ...
        root.BOT = root.BOT || { role: "warrior", config: { name: "Dorg" } };
        ...
        root.BOT_STATUS = {
            ...
            farm: (root.BOT.config && root.BOT.config.farm && root.BOT.config.farm.monster) || "?"
        };
        log("BOT_STATUS = " + JSON.stringify(root.BOT_STATUS));
        try {
            if (typeof set_message === "function") set_message("DORG " + root.BOT_STATUS.combat);
        } catch (e) { /* ignore */ }
        ...
        root.BOT_DORG_INTERVAL = setInterval(function () {
            mainTick();
        }, TICK_MS);

        log("DORG_Main running. In CODE console type: BOT_STATUS");
    } catch (error) {
        root.BOT_STATUS = { main: "DORG_Main", phase: "CRASHED", error: safeError(error) };
        log("DORG_Main CRASHED: " + safeError(error), "#FF8080");
    }
})();
